...
export default function Restaurants({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      async function getData() {
        ...
        if (userResponse.statusResponse !== false) {
          if (userResponse.typeUser === "property") {
            setUserRestaurant(true);
          }
          else {
            setUserRestaurant(false);
  ...
  const handleLoadMore = async () => {
    ...
    setLoading(true);
    const response = await getMoreRestaurant(
      limiteRestaurants,
      startRestaurants
    );
    if (response.statusResponse) {
    ...
    <View style={styles.viewBody}>
      {size(restaurants) > 0 ? (
        ...
        <View style={styles.notFoundViewStyle}>
          <Text style={styles.notRestaurantText}>Not exist Restaurants</Text>
        </View>
      )}
      {userRestaurant && (
        <Icon
