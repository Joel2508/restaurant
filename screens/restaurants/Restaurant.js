...
export default function Restaurant({ navigation, route }) {
  ...
  const [userRestaurant, setUserRestaurant] = useState(false);
    ...
    useCallback(() => {
      (async () => {
        ...
        if (response.statusResponse) {
          ...
          if (response.document.createBY === currentUser.uid) {
  ...
  const remoteFavoriteDoc = async () => {
    ...
    if (response.statusResponse) {
      ...
      toastRef.current.show(
        "Not can't delete the restaurant that favorite.",
        3000
      );
    }
  };
  ...
  const addFavorite = async () => {
    if (!userLogger) {
      toastRef.current.show(
        "If want add this new restaurant you must at log in.",
        3000
      );
      return;
    }
      ...
      <KeyboardAwareScrollView style={styles.viewScrollSendMessage}>
        <View style={styles.modalNotificationStyle}>
          <Text style={styles.textStyle}>
            Send a message to lovers {restaurant.name}
          </Text>
          <Input
...
function RestaurantInfo({
  ...
  const listInfo = [
    {
      ...
      iconRigth: "message-text-outline",
    },
    ...
    if (text === "phone") {
      ...
      if (currentUser) {
        sendEmail(
          email,
          "Interesting",
          `I am ${currentUser.displayName}, I am interested in your services`
        );
      } else {
  ...
  const actionRigth = (text) => {
    if (text === "phone") {
      ...
      if (currentUser) {
        ...
        sendWhastApp(
          phone,
          "Interesting " + ` I am interested in your services`
        );
      }
    } else if (text == "addres") {
      if (userRestaurant === true) {
        setModalNotification(true);
      } else {
        toastRef.current.show("This restaurant don't is your", 3000);
        return;
      }
    }
  };
  ...
  return (
    <View style={styles.viewRestaurantInfo}>
      <Text style={styles.infoTitleRestaurant}>Information the restaurant</Text>
      <MapRestaurant name={name} location={location} height={150} />
      {map(listInfo, (item, index) => (
        <ListItem key={index} style={styles.containerList}>
          <Icon
            type="material-community"
            name={item.iconLeft}
            color="#3c3c4c"
